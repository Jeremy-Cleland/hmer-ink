...
def compute_expression_recognition_rate(predictions: List[str], targets: List[str]) -> float:
    """
...
def compute_symbol_accuracy(predictions: List[str], targets: List[str]) -> float:
    ...
    for p_tokens, t_tokens in zip(pred_tokens, target_tokens):
        total_correct += sum(p == t for p, t in zip(p_tokens[:len(t_tokens)], t_tokens))
...
def compute_metrics(predictions: List[str], targets: List[str], 
    ...
    available_metrics = {
        ...
        'expression_recognition_rate': lambda ps, ts: compute_expression_recognition_rate(ps, ts),
    ...
    for metric in metrics:
        if metric in sample_metrics:
            metric_values = [available_metrics[metric](p, t) for p, t in zip(predictions, targets)]
