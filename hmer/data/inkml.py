...
class InkmlParser:
    ...
    def parse_inkml(file_path: str) -> Dict:
        ...
        result = {
            ...
            'normalized_label': annotations.get('normalizedLabel', annotations.get('label', '')),
            'metadata': {
                'sample_id': annotations.get('sampleId', os.path.basename(file_path).split('.')[0]),
    ...
    def normalize_strokes(strokes: List[np.ndarray], 
                         ...
                         time_range: Optional[Tuple[float, float]] = None) -> List[np.ndarray]:
        """
        ...
        for stroke in strokes:
            ...
            normalized_stroke[:, 0] = normalized_stroke[:, 0] * (x_range[1] - x_range[0]) + x_range[0]
            
            # Normalize y coordinates
            normalized_stroke[:, 1] = (stroke[:, 1] - y_min) / y_scale
            normalized_stroke[:, 1] = normalized_stroke[:, 1] * (y_range[1] - y_range[0]) + y_range[0]
            ...
            if time_range and stroke.shape[1] > 2:
                ...
                normalized_stroke[:, 2] = normalized_stroke[:, 2] * (time_range[1] - time_range[0]) + time_range[0]
