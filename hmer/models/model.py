...
class HMERModel(nn.Module):
    ...
    def forward(self, 
        ...
        if target_lengths is not None:
            ...
            tgt_padding_mask = torch.arange(tgt_len, device=target_seq.device).expand(batch_size, tgt_len) >= target_lengths.unsqueeze(1)
        
        # Prepare autoregressive mask if needed
        tgt_mask = None
        if self.autoregressive:
            tgt_len = target_seq.shape[1]
            tgt_mask = self.decoder.generate_square_subsequent_mask(tgt_len).to(target_seq.device)
    ...
    def generate(self, 
                ...
                input_lengths: Optional[torch.Tensor] = None,
                max_length: int = 128,
                beam_size: int = 4) -> Tuple[List[List[int]], List[float]]:
        """
        ...
        with torch.no_grad():
            ...
            for batch_idx in range(batch_size):
                ...
                # Run beam search
                beams, scores = self._beam_search(
                    mem, mask, max_length, beam_size
                )
    ...
    def _beam_search(self, 
                    ...
                    max_length: int,
                    beam_size: int) -> Tuple[List[List[int]], List[float]]:
        """
        ...
        for step in range(max_length):
            ...
            for beam_idx, (tokens, score) in enumerate(beams):
                ...
                # Apply decoder
                if self.autoregressive:
                    tgt_mask = self.decoder.generate_square_subsequent_mask(tokens.shape[1]).to(device)
                    logits = self.decoder(
                        tokens, 
                        expanded_memory[beam_idx:beam_idx+1],
                        tgt_mask=tgt_mask,
                        memory_key_padding_mask=expanded_mask[beam_idx:beam_idx+1] if expanded_mask is not None else None
                    )
                else:
    ...
    def save_checkpoint(self, path: str, epoch: int, optimizer: Any = None, 
                      scheduler: Any = None, best_metric: float = None):
        """
