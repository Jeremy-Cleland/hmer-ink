...
parser.add_argument("--metrics_dir", type=str, default="outputs/training_metrics",
...
metrics_file = os.path.join(args.metrics_dir, "training_metrics.json")
...
if metrics_exist:
    ...
    if len(df) > 0:
        ...
        error_examples_files = [f for f in os.listdir(args.metrics_dir) if f.startswith("error_examples_epoch_") and f.endswith(".json")]
        ...
        if error_examples_files:
            ...
            try:
                with open(os.path.join(args.metrics_dir, latest_error_file), "r") as f:
