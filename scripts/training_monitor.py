...
class TrainingMonitor:
    ...
    def _plot_training_curves(self, df: pd.DataFrame):
        ...
        plt.tight_layout()
        plt.savefig(os.path.join(self.log_dir, "plots", "training_curves.png"), dpi=100, bbox_inches="tight")
        plt.close()
    ...
    def _plot_metric_correlations(self, df: pd.DataFrame):
        ...
        plt.tight_layout()
        plt.savefig(os.path.join(self.log_dir, "plots", "metric_correlations.png"), dpi=100, bbox_inches="tight")
        plt.close()
    ...
    def _visualize_error_examples(self):
        ...
        plt.tight_layout()
        plt.savefig(os.path.join(self.log_dir, "plots", f"error_analysis_epoch_{latest.get('epoch', 0)}.png"), 
                   dpi=100, bbox_inches="tight")
        plt.close()
        
        # Save the detailed examples
        with open(os.path.join(self.log_dir, f"error_examples_epoch_{latest.get('epoch', 0)}.json"), "w") as f:
...
if __name__ == "__main__":
    ...
    print("This script is now integrated with the CLI. Use 'python cli.py monitor' instead.")
