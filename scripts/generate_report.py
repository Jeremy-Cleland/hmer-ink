...
from hmer.config import load_config, get_device


def generate_report(model_path: str, config_path: str, output_path: str, num_samples: int = 20):
    """
    ...
    if not os.path.exists(tokenizer_path):
        tokenizer_path = os.path.join(config['output'].get('checkpoint_dir', 'outputs/checkpoints'), 'vocab.json')
    ...
    eval_results_path = os.path.join(os.path.dirname(model_path), 'evaluation_test.json')
    ...
    if not metrics:
        ...
        sample_indices = random.sample(range(len(test_dataset)), min(100, len(test_dataset)))
        predictions = []
        targets = []
        ...
        with torch.no_grad():
            for idx in tqdm(sample_indices):
                ...
                input_lengths = torch.tensor([input_seq.shape[1]], dtype=torch.long).to(device)
    ...
    with torch.no_grad():
        for idx in tqdm(sample_indices):
            ...
            for stroke in strokes:
                ...
                plt.scatter(stroke[0, 0], -stroke[0, 1], color='green', s=20)  # Start point
                plt.scatter(stroke[-1, 0], -stroke[-1, 1], color='red', s=20)  # End point
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Generate a comprehensive HTML report for model evaluation')
    parser.add_argument('--model', type=str, required=True, help='Path to model checkpoint')
    parser.add_argument('--config', type=str, default='configs/default.yaml', help='Path to configuration file')
    parser.add_argument('--output', type=str, default='outputs/report.html', help='Path to save report HTML')
    parser.add_argument('--samples', type=int, default=20, help='Number of random samples to include in report')
