...
def expand_model(
    ...
    small_model, checkpoint = HMERModel.load_checkpoint(src_checkpoint, map_location=device)
    ...
    print(f"Parameter increase: {large_params - small_params:,} (+{(large_params/small_params - 1)*100:.1f}%)")
            ...
            if large_state_dict[name].shape == param.shape:
                ...
                if "embedding" in name or "output_projection" in name:
                    if len(param.shape) == 2 and len(large_state_dict[name].shape) == 2:
                        ...
                        large_state_dict[name][:min_dim0, :min_dim1] = param[:min_dim0, :min_dim1]
    ...
    if "optimizer" in checkpoint:
        print("Note: Optimizer state from source model cannot be transferred due to parameter count differences")
...
def main(
    src_checkpoint: str = typer.Argument(..., help="Path to source checkpoint (smaller model)"),
    dst_checkpoint: str = typer.Argument(..., help="Path to save the expanded model"),
    config: str = typer.Option("configs/fast_expanded.yaml", "--config", "-c", help="Configuration for expanded model"),
    device: str = typer.Option("", "--device", "-d", help="Device to use (default: from config)"),
    dry_run: bool = typer.Option(False, "--dry-run", help="Only show model comparison without saving"),
):
    ...
    if dry_run:
        ...
        decoder = get_decoder({"decoder": expanded_config["model"]["decoder"]}, vocab_size)
