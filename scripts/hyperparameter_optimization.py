...
import wandb
from hmer.config import load_config, update_config
...
def run_sweep(config_path: str, experiment_name: str, num_runs: int = 10):
    ...
    sweep_config = {
        ...
        'metric': {
            'name': 'val_expression_recognition_rate',
            'goal': 'maximize'
        },
        'parameters': {
            'learning_rate': {
    ...
    def train_model():
        ...
        updated_config['model']['encoder']['embedding_dim'] = params.encoder.embedding_dim
        ...
        updated_config['data']['augmentation']['scale_range'] = params.augmentation.scale_range
        updated_config['data']['augmentation']['rotation_range'] = params.augmentation.rotation_range
        
        # Make sure model decoder embedding_dim matches encoder embedding_dim
        updated_config['model']['decoder']['embedding_dim'] = updated_config['model']['encoder']['embedding_dim']
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Run hyperparameter optimization using W&B Sweeps')
    parser.add_argument('--config', type=str, default='configs/default.yaml', help='Path to base configuration file')
    parser.add_argument('--experiment', type=str, default='hpo-hmer-ink', help='Name for the sweep experiment')
    parser.add_argument('--num-runs', type=int, default=10, help='Number of runs to perform')
